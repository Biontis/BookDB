...
const fetchBooks = async (query: string): Promise<Book[]> => {
  try {
    const res = await fetch(
      ...
      )}&maxResults=20&orderBy=relevance&langRestrict=tr`
    );
...
const InteractiveStarRating: React.FC<StarRatingProps> = ({ value, onChange, size = 24, readOnly = false }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const exactValue = percent * 5;
    let roundedValue = Math.ceil(exactValue * 2) / 2;
    if (roundedValue < 0.5) roundedValue = 0.5;
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto pt-4 border-t border-zinc-800">
          ...
          >
            ...
            {userRating > 0 ? `Puanın: ${userRating}` : "Puan Ver"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden relative">
        ...
        <div className="p-8 flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-1">Kitabı Puanla</h2>
          <p className="text-zinc-400 mb-8 line-clamp-2">{book.title}</p>
          
          <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800/50 mb-8">
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setErrorMsg(error.message || "İşlem başarısız. Bilgilerinizi kontrol edip tekrar deneyin.");
      }
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        setErrorMsg("HATA: Mevcut site adresi Firebase'de izin verilen domainler listesinde yok! Lütfen Firebase Console -> Authentication -> Settings -> Authorized Domains kısmına bu önizleme linkini ekleyin.");
      } else if (error.code === 'auth/popup-blocked') {
        setErrorMsg("Tarayıcınız pencereyi engelledi. Sağ üstten siteyi 'Yeni Sekmede Aç' butonuna tıklayarak açmayı deneyin.");
      } else if (error.code === 'auth/popup-closed-by-user') {
        setErrorMsg("Giriş yapmadan pencereyi kapattınız.");
      } else {
        setErrorMsg(error.message || "Google ile giriş yapılamadı. Lütfen siteyi yeni sekmede açmayı deneyin.");
      }
    }
  };
...
export default function App() {
  ...
  // Authentication Initialization (Firebase)
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        ...
        setUser(null);
        // Load global (anonymous) data if no user
  ...
  // user is managed by Firebase Auth, we don't need to manually store it in localStorage anymore.
              ...
              <div className="p-6 md:p-8 flex flex-col justify-center flex-1 relative">
                ...
                <div className="mt-auto flex flex-wrap gap-4">
                  ...
                  >
                    ...
                    {userRatings[featuredBook.id] > 0 ? `Puanın: ${userRatings[featuredBook.id]}` : "Puan Ver"}
                  </button>
                  <button 
